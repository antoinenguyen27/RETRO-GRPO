...
class Stage1Trainer:
    ...
    def _generate_rollout_batch(self, unwrapped_model, batch_rows: list[dict]) -> RolloutBatch:
    ...
    def _prepare_window(
        ...
        for batch_rows in microbatches:
            rollout = self._generate_rollout_batch(unwrapped_model, batch_rows)
            rollout.group_ids = [group_id + group_offset for group_id in rollout.group_ids]
            group_offset += len(batch_rows)
    ...
    def _summarize_rollout_metrics(
        ...
        if raw.get("retro_prompt_count", 0.0) > 0.0:
            metrics["retro/scheduled_conditioning_rate"] = (
                raw.get("long_memory_injected_prompt_count", raw["retro_prompt_count"])
                / raw["retro_prompt_count"]
            )
        ...
        if raw.get("long_memory_prompt_count", 0.0) > 0.0:
            prompt_count = raw["long_memory_prompt_count"]
            metrics["long/memory_retrieval_rate"] = (
                raw["long_memory_existing_prompt_count"] / prompt_count
            )
            metrics["long/memory_refresh_rate"] = (
                raw["long_memory_refreshed_prompt_count"] / prompt_count
            )
            metrics["long/memory_usage_rate"] = (
                raw["long_memory_injected_prompt_count"] / prompt_count
            )
            metrics["long/memory_dropout_rate"] = (
                raw["long_memory_dropped_prompt_count"] / prompt_count
            )
            metrics["long/stale_memory_rate"] = (
                raw["long_memory_stale_prompt_count"] / prompt_count
            )
            metrics["long/current_failure_rate"] = (
                raw["long_memory_current_failure_prompt_count"] / prompt_count
            )
            metrics["long/memory_created_count"] = raw["long_memory_created_count"]
            metrics["long/memory_updated_count"] = raw["long_memory_updated_count"]
            metrics["long/memory_changed_count"] = raw["long_memory_changed_count"]
            metrics["long/memory_decayed_count"] = raw["long_memory_decayed_count"]
            metrics["long/memory_retired_count"] = raw["long_memory_retired_count"]
            metrics["long/memory_tokens_total"] = raw["long_memory_tokens"]
            metrics["long/memory_tokens_mean"] = (
                raw["long_memory_tokens"] / raw["long_memory_injected_prompt_count"]
                if raw["long_memory_injected_prompt_count"] > 0.0
                else 0.0
            )
            metrics["long/memory_tokens_max"] = raw["long_memory_tokens_max"]
            metrics["long/memory_available_tokens"] = raw["long_memory_available_tokens"]
            metrics["long/memory_refresh_generate_s"] = raw[
                "long_memory_refresh_generate_s"
            ]
            metrics["long/memory_stability_mean"] = (
                raw["long_memory_stability_total"] / raw["long_memory_refreshed_prompt_count"]
                if raw["long_memory_refreshed_prompt_count"] > 0.0
                else 0.0
            )
...
class BaselineTrainer(Stage1Trainer):
    ...
    def _generate_rollout_batch(self, unwrapped_model, batch_rows: list[dict]) -> RolloutBatch:
        return run_baseline_rollouts(
            model=unwrapped_model,
            tokenizer=self.tokenizer,
            prompt_messages_batch=[row["prompt"] for row in batch_rows],
...
class RetroTrainer(Stage1Trainer):
    ...
    def __init__(
        ...
        super().__init__(model, tokenizer, train_dataset, config, run_name, output_dir)
        self.prompt_solve_emas: dict[str, float] = {}
        self.memory_store = {}
    ...
    def _generate_rollout_batch(self, unwrapped_model, batch_rows: list[dict]) -> RolloutBatch:
        return run_retro_rollouts(
            ...
            prompt_solve_emas=self.prompt_solve_emas,
