...
MEMORY_REFRESH_TEMPLATE = """\
You are refreshing a prompt-local failure memory for the same problem.
Rewrite the prior memory and the fresh failure summary into one compact \
summary in 6-10 sentences. Keep only the information that still appears \
useful for describing recurring failed strategy classes. When the old and \
new evidence disagree, prioritize the fresh summary. Do not evaluate \
correctness. Do not suggest alternatives.
...
def build_memory_refresh_prompt(
    question: str,
    prior_memory: str,
    fresh_summary: str,
) -> str:
    return MEMORY_REFRESH_TEMPLATE.format(
        problem_text=question,
        prior_memory=prior_memory.strip(),
        fresh_summary=fresh_summary.strip(),
    )
...
@torch.no_grad()
def generate_refreshed_memories(
    model,
    tokenizer,
    requests: list[MemoryRefreshRequest],
    ...
    """Refresh stored memories with new failure summaries in batch."""
    if not requests:
        return []
    ...
    messages_batch = [
        [
            ...
            {
                "role": "user",
                "content": build_memory_refresh_prompt(
                    request.question,
                    request.prior_memory,
                    request.fresh_summary,
                ),
            },
        ]
