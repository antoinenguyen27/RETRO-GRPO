"""Modal app for long-memory RETRO-GRPO training.
...
Entry points:
    ...
    modal run train.py --mode smoke_long_retro --steps 10
"""
...
app = modal.App("long-retro-grpo-train")
...
def train_baseline():
    trainer = _build_trainer(
        mode="baseline",
        run_name="long_baseline_grpo",
        output_dir="/models/long_baseline_checkpoints",
    )
    trainer.train()
    _save_final_checkpoint(trainer, "/models/long_baseline_final")
    model_volume.commit()
...
def train_retro():
    trainer = _build_trainer(
        mode="retro",
        run_name="long_retro_grpo",
        output_dir="/models/long_retro_checkpoints",
    )
    trainer.train()
    _save_final_checkpoint(trainer, "/models/long_retro_final")
    model_volume.commit()
...
def smoke_test(mode: str = "baseline", steps: int = 10):
    ...
    dataset = load_from_disk(TRAIN_DATA_DIR)
    if mode == "long_retro":
        # Force prompt reuse so the smoke test actually exercises memory retrieval,
        # refresh, decay, and retirement across appearances.
        dataset = dataset.select(range(min(len(dataset), 48)))
    ...
    trainer = trainer_cls(
        ...
        train_dataset=dataset,
        config=config,
        run_name=f"long_smoke_{mode}",
        output_dir=f"/models/long_smoke_{mode}",
    )
...
def main(mode: str = "baseline", steps: int = -1):
    if mode == "baseline":
        ...
        train_retro.remote()
    elif mode == "smoke_long_retro":
        smoke_test.remote(mode="long_retro", steps=steps if steps > 0 else 10)
        ...
        raise ValueError(
            "Unknown mode: "
            f"{mode}. Use baseline, retro, smoke_baseline, smoke_retro, "
            "or smoke_long_retro."
        )
